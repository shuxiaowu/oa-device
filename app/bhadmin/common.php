<?php
  ...
  function btn($conf = array())
  {
	  ...
	  return '<button type="' . $type . '" tabindex="999" data-id="'.$id.'" value="' . $vals . '" name="' . $name . '"' . $ckurl . ' class="btn ' . $scene . $size . $add . '" ' . $tips . $ban . '>' . $icon . $vals . '</button>';
  }
  ...
  function tabstyle()
  {
	  return 'table table-hover table-bordered';//table-bordered 
  }
  ...
  {
	  if ($id) {
		  if ( !$data = cache('showadminauth'.$id) ) {
		  	 $data = db('adminauth')->field("*")->where('tid=' . $id)->order('ord ASC')->select();
		  	 cache('showadminauth'.$id,$data);
		  }
		  if ($data) {
			  $str = '';
  ...
  function tplUrl($url = '')
  {
	  if ($url == '') {
		  ...
		  $uArr = explode(",", $url);
		  if (count($uArr) > 1) {
			  return url('bhadmin/'.$uArr[0], $uArr[1]);
		  } else {
			  return url('bhadmin/'.$uArr[0]);
		  }
	  }
  }
  ...
  function uppic($config = array())
  {
	  ...
	  if ($iscrop) {
		  ...
		  $cjhtml  = btn(array('vals' => '裁剪', 'add' => 'btn-cut', 'round' => 1, 'scene' => 'default', 'icon' => 'scissors', 'tips' => '点击裁剪图片'));
		  $cjhtml .= '&nbsp;'.btn(array('vals' => '美化', 'add' => 'btn-magic', 'round' => 1, 'scene' => 'default', 'faicon' => 'magic', 'tips' => '点击美化图片'));
		  $tips    = ($tips != '') ? '<div class="uptips text-warning">' . icon('info-sign') . ' ' . $tips . $cjhtml . $choice . '</div>' : '<div class="uptips text-warning">' . $cjhtml . $choice . '</div>';
	  } else {
  ...
  //多图上传
  function upatlas($file = '', $tips = '',$type = 1, $piclimit = 5)
  {
	  $tips = ($tips != '') ? '<div class="uptips text-warning">' . icon('info-sign') . ' ' . $tips . '</div>' : '';
	  if ($file == '') {
		  return '<div class="bh-newupload upload-skin"><input id="upatlas" name="Filedata" type="file" multiple><span class="uploadloadings"><span class="glyphicon glyphicon-picture"><b>选择上传</b></span></span></div>' . $tips . '<div class="upupatlas"><h3>图集缩略图，允许上传图片 <span class="piclimit">' . $piclimit . '</span> 张</h3><div class="atlaspiclist"></div></div>';
	  } else {
		  $fileArr = ($type==1) ? unserialize($file) : explode(',',$file);
		  $myatlas = '';
		  if (count($fileArr) > 0) {
		  ...
		  return '<div class="bh-newupload upload-skin"><input id="upatlas" name="Filedata" type="file" multiple><span class="uploadloadings"><span class="glyphicon glyphicon-picture"><b>选择上传</b></span></span></div>' . $tips . '<div class="upupatlas" style="display:block;"><h3>图集缩略图，允许上传图片 <span class="piclimit">' . $limit . '</span> 张</h3><div class="atlaspiclist">' . $myatlas . '</div></div>';
	  }
  }
  ...
  {
	  if ($val == 1) {
		  return '<div class="btn-group" data-id="' . $id . '" data-tables="' . $tables . '" data-field="' . $field . '" data-tip1="' . $tip[0] . '" data-tip2="' . $tip[1] . '"><button type="button" tabindex="999" class="btn btn-roundeds btn-info btn-xs btn-enabled" data-mark="1" data-toggle="tooltip" data-placement="top" title="点击' . $tip[0] . '数据">' . $tip[0] . '</button><button type="button" tabindex="999" class="btn btn-default btn-roundeds btn-disabled btn-xs" data-mark="2" data-toggle="tooltip" data-placement="top" title="点击' . $tip[1] . '数据">' . $tip[1] . '</button></div>';
	  } else {
  ...
  function dropdown($data, $id = 0, $sel = '请选择类别', $name = "inftype")
  {
	  ...
	  if ($data == 3) $data = array(0 => array('Id' => 2, 'topic' => '审核'), 1 => array('Id' => 1, 'topic' => '非审核'));
  ...
  function searchField($conf = array())
  {
	  ...
	  $tables  = isset($conf['tables'])    ? $conf['tables']    : '';      //表
	  ...
	  $bwhere  = isset($conf['where'])     ? $conf['where']     : '1=1';   //默认基础条件
	  $val     = isset($conf['val'])       ? $conf['val']       : 0;       //默认值
	  if ($name != '' && $tables != '' && $s_field != '') {
		  $list = '';
		  $cachename = md5($tables.$s_field.$bwhere);
		  if ( !$data = cache($cachename) ) {
		  	  $data = db($tables)->field('Id,' . $s_field)->where($bwhere)->limit($limit)->select();
		  }
		  if ($data) {
			  cache($cachename,$data,3600);
			  foreach ($data as $skey => $sval) {
				  $list .= '<li class="active-result" data-id="' . $sval['Id'] . '">' . $sval[$field] . '</li>';
			  }
		  } else {
			  $list = '<li class="active-result" data-id="0">暂无数据</li>';
		  }
		  return '<div class="chosen-container chosen-container-single" style="width:' . $width . 'px;">
				   ...
				   <div class="chosen-drop">
					<div class="chosen-search">
					 <input type="hidden" value="'.$val.'" class="chose-keyid" name="' . $name . '">
					 <input type="text" class="chosen-keys" data-field="' . $field . '" data-sfield="' . $s_field . '" data-tables="' . $tables . '" data-where="'.($bwhere).'" autocomplete="off">
					</div>
  ...
  {
	  if ($topmenuid) {
		  if ( !$mmenu = cache('admintopmenu'.$topmenuid) ) {
			  $mmenu = db('adminauth')->field('Id,name,icon,isext,linkurl,isopen,title')->where('1=1 AND tid=' . $topmenuid)->cache(true, 60 * 60)->order('ord ASC')->select();
			  if ($mmenu) {
				  foreach ($mmenu as $key => $val) {
					  $mmenu[$key]['menu'] = db('adminauth')->field('name,icon,isext,linkurl,title,Id')->where('1=1 AND isspecial=0 AND tid=' . $val['Id'])->cache(true, 60 * 60)->order('ord ASC')->select();
				  }
			  } else {
				  $mmenu = [];
			  }
			  cache('admintopmenu'.$topmenuid,$mmenu);
		  }
		  return $mmenu;
	  }
  ...
  function getAuth()
  {
	  ...
	  if ($loginstatue['status']) {
		  ...
		  if ( !$auth = cache('adminauth'.$admindepid) ) {
		  	 $auth = db('admindepartment')->field('auth')->where(array('Id' => $admindepid))->find();
			 cache('?adminauth'.$admindepid,$auth);
		  }
		  if ($auth) {
			  return $auth['auth'];
		  } else {
  ...
  function getseting()
  {
	  ...
	  $sys_info['loginwarning'] = cookie("loginWarning");
	  $sys_info['passWarning']  = cookie("passWarning");
	  $sys_info['c_site']       = getdata('systemconfig',1,'c_site');
	  if ( $sys_info['debug'] )           $count++;
	  if ( $sys_info['app_trace'] )       $count++;
	  if ( cookie("loginWarning") != '' ) $count++;
	  if ( cookie("passWarning") != '' )  $count++;
	  if ( $sys_info['c_site'] )          $count++;
  ...
  {
	  return '<span class="tdkmodal" data-id="' . $id . '" data-tables="' . $table . '">' . faicon('cog') . '</span>';
  }
  ...
  {
	  if ($uri != '') {
		  $conf = db('seoconf')->field('*')->where('Id=1')->cache(true,3000)->find();
   ...
   * @return string
   */
  function bhdrops($params = array())
  {
      $url   = isset($params['url'])  ? $params['url'] : 'admin/ajaxdrops';
      $scene = isset($params['scene'])? $params['scene'] : 'address';
      $data  = isset($params['data']) ? $params['data'] : array();
      $names = isset($params['name']) ? $params['name'] : array();
      $vals  = isset($params['vals']) ? $params['vals'] : array();
      $places= isset($params['place']) ? $params['place'] : array();
      $level = isset($params['level']) ? $params['level'] : 1;
      $html = '<div class="select-item-list">';
      if($level>=1){
          $name = isset($names[0]) ? $names[0] : 'ctag';
          $place = isset($places[0]) ? $places[0] : '请选择';
          $value = isset($vals[0]) ? $vals[0] : 0;
          $map  = isset($data[0]) ? $data[0] : array();
          $html .= '<div class="select-item" data-index="0" data-scene="'.$scene.'" data-url="'.$url.'"><select class="form-control select-event" name="'.$name.'"><option value="0">'.$place.'</option>';
          foreach ($map as $k=>$v){
              $html .= '<option value="'.$v['Id'].'" '.($value==$v['Id'] ? 'selected="selected"':'').'>'.$v['topic'].'</option>';
          }
          $html .= '</select></div>';
      }
      if($level>=2){
          $name = isset($names[1]) ? $names[1] : 'mtag';
          $place= isset($places[1])? $places[1] : '请选择';
          $value= isset($vals[1]) ? $vals[1] : 0;
          $map  = isset($data[1]) ? $data[1] : array();
          $html .= '<div class="select-item" data-index="1" data-scene="'.$scene.'" data-url="'.$url.'"><select class="form-control select-event" name="'.$name.'"><option value="0">'.$place.'</option>';
          foreach ($map as $k=>$v){
              $html .= '<option value="'.$v['Id'].'" '.($value==$v['Id'] ? 'selected="selected"':'').'>'.$v['topic'].'</option>';
          }
          $html .= '</select></div>';
      ...
      if($level>=3){
          $name = isset($names[2]) ? $names[2] : 'stag';
          $place= isset($places[2])? $places[2] : '请选择';
          $value= isset($vals[2]) ? $vals[2] : 0;
          $map  = isset($data[2]) ? $data[2] : array();
          $html .= '<div class="select-item" data-index="2" data-scene="'.$scene.'" data-url="'.$url.'"><select class="form-control select-event" name="'.$name.'"><option value="0">'.$place.'</option>';
          foreach ($map as $k=>$v){
              $html .= '<option value="'.$v['Id'].'" '.($value==$v['Id'] ? 'selected="selected"':'').'>'.$v['topic'].'</option>';
          }
          $html .= '</select></div>';
      ...
      if($level>=4){
          $name = isset($names[3]) ? $names[3] : 'dtag';
          $place= isset($places[3])? $places[3] : '请选择';
          $value= isset($vals[3]) ? $vals[3] : 0;
          $map  = isset($data[3]) ? $data[3] : array();
          $html .= '<div class="select-item" data-index="3" data-scene="'.$scene.'" data-url="'.$url.'"><select class="form-control select-event" name="'.$name.'"><option value="0">'.$place.'</option>';
          foreach ($map as $k=>$v){
              $html .= '<option value="'.$v['Id'].'" '.($value==$v['Id'] ? 'selected="selected"':'').'>'.$v['topic'].'</option>';
          }
          $html .= '</select></div>';
  ...
  function bhcopy($tables='',$id=0){
  	  return '<span class="glyphicon glyphicon-copy bhcopy" data-tables="'.$tables.'" data-id="'.$id.'"'.tooltip('点击复制此数据').'></span>';
  }
  
  function couponExpire($expiretime,$expireday){
      if(config('couponmode')==2){
          return '领取后'.$expireday.'天过期';
      }else{
          if(time()>strtotime($expiretime)){
              return '<span class="btn btn-xs btn-danger">过期</span>'; 
          }else{
              return $expiretime; 
          }
      }
