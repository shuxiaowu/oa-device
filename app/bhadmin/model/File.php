...
class File{
	...
	public function picupload($act = ''){
		...
		$movepath   = '';
		$dpath      = ($isproimg) ? 'products' : date('Ymd');
		...
		if ($isproimg) $movepath = ROOT_PATH . config('upload_path') . 'images/products';
		$movepaths = ($isproimg) ? $movepath : $movepath . '/' . $dpath;
    ...
    //获取配置
	private function upsetting()
	{
		if (!$uploadField = cache('system_uploads')) {
			$field = 'iswater,fontpos,waterpos,waterpic,fonttext,fontsize,fontcolor,facetype,rotation,wateralpha,picsuffix,filesuffix,picsize,filesize,picmaxwidth,picmaxsize,cropwidth';
			$data = Db::name('systemconfig')->field($field)->where('Id',1)->find();
			cache('system_uploads', $data);
			return $data;
		} else {
			return $uploadField;
		}
    ...
    //获取文件
	public function getpic($spath = '', $page = 1, $psize = 15)
	{
		$sdata = array();
