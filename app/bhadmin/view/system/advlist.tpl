...
<block name="main">
  <div class="pubmain">
    <div class="panel-body">
      <div class="u-plus">{:btn(array('vals'=>'添加广告','size'=>3,'faicon'=>'plus','scene'=>'primary','url'=>url('system/advadd')))}</div>
      <form name="pubserch" method="get" action="">
        <div class="search"> 广告名称：
          <input type="text" class="text" name="topic" placeholder="广告名称" style="width:150px;">
          &nbsp;广告位置：{:dropdown($advtype,0,'选择广告位置','ctag')}
          &nbsp;是否启用：{:dropdown(1,0,'请选择','enabled')}
          <input type="hidden" value="{$dshow['table']}" name="tables">
          &nbsp;{:btn(array('vals'=>'查询','type'=>'submit','icon'=>'search','name'=>'searchdata','round'=>1,'size'=>4,'scene'=>'primary'))} </div>
      </form>
          ...
          <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="{:tabstyle()}">
            ...
            <volist name="data" id="obj">
              <tr class="maintr">
                ...
                <td align="left" valign="middle">{:btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','round'=>1,'url'=>url('system/advmod','tables='.$dshow['table'].'&id='.$obj['Id'])))}</td>
              </tr>
            </volist>
            <tr>
              <td height="37" align="center" valign="middle">{:ckall(2)}</td>
              <td height="35" colspan="9" align="left" valign="middle"> {:btn(array('vals'=>'删除','type'=>'submit','round'=>1,'name'=>'deldata','icon'=>'trash','scene'=>'danger'))} <font class="text-warning">&nbsp;{:icon('warning-sign')} 提示，任何形式删除的数据都无法找回，请谨慎操作！</font> {$dshow['pageshow']} </td>
            </tr>
          </table>
        </form>
        <else/>
