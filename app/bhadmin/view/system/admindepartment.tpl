...
<block name="main">
  <div class="pubmain">
    <div class="panel-body">
      <div class="u-plus">{:btn(array('vals'=>'添加部门','size'=>3,'icon'=>'plus','scene'=>'primary','url'=>url('system/admindepartmentadd')))}</div>
      <form name="publist" method="post" action="" onSubmit="return pubdel(document.publist)">
        <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="{:tabstyle()}">
          ...
          <tr>
            ...
            <td height="35" colspan="6" align="left" valign="middle"> {:btn(array('vals'=>'删除','type'=>'submit','name'=>'deldata','round'=>1,'icon'=>'trash','scene'=>'danger'))}
              {$dshow['pageshow']} </td>
          </tr>
        </table>
      </form>
      <div class="alert alert-primary" style="margin:10px auto;">{:icon('warning-sign')} 提示，删除无法恢复，请谨慎操作。</div>
    </div>
  </div>
</block>
