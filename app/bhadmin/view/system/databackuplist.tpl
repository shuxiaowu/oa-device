      ...
      <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="{:tabstyle()}">
        ...
        <volist name="data" id="obj">
          <tr class="maintr">
            ...
            <td align="left" valign="middle"> {:btn(array('vals'=>'下载','icon'=>'download','tips'=>'点击下载此备份文件','round'=>1,'url'=>url('system/downdataup','downpath='.$obj['path'],'')))}
              <if condition="$datasys['isdel'] eq 1"> <a href="javascript:void(0)" data-path="{$obj['path']}" class="deldataup">{:btn(array('vals'=>'删除','icon'=>'trash','round'=>1,'tips'=>'点击删除该备份文件','scene'=>'danger'))}</a> </if>
              <if condition="$datasys['isre'] eq 1"> <a href="javascript:void(0)" data-path="{$obj['path']}" class="redataup">{:btn(array('vals'=>'还原','icon'=>'refresh','round'=>1,'tips'=>'点击还原该份备份数据','scene'=>'success'))}</a> </if></td>
          </tr>
        </volist>
      </table>
      <div class="alert alert-danger" style="margin:10px auto;">{:icon('warning-sign')} 提示，任何形式删除的数据都无法找回，请谨慎操作！</div>
    </div>
  </div>
