...
<block name="main">
  <div class="pubmain">
    <div class="panel-body">
      <div class="u-plus">{:btn(array('vals'=>'添加资料','size'=>3,'icon'=>'plus','scene'=>'primary','url'=>url('website/dataadd','sty='.$sty.'&tables='.$dshow['table'].'&martables='.$dshow['martable'])))}</div>
      <form name="pubserch" method="get" action="{:url('website/datalist')}">
        <div class="search"> 资料名称：
          <input type="text" class="text" name="topic" placeholder="资料名称" style="width:150px;">
          ...
          <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="{:tabstyle()}">
            ...
            <volist name="data" id="obj">
              <tr class="maintr">
                ...
                <td align="left" valign="middle"> {:bhcopy($dshow['table'],$obj['Id'])}
                  <if condition="$obj['pic'] neq ''"> <a href="javascript:void(0)"{:imgshow($obj['pic'])}>{:icon('picture')}</a> </if>
                ...
                <td align="center" valign="middle"><a href="javascript:void(0)" data-url="{:url('Website/article','id='.$obj['Id'].'&tables='.$dshow['table'])}" data-title="{$obj['topic']}" class="showdata"> {:btn(array('vals'=>'预览','icon'=>'globe','scene'=>'success','round'=>1,'tips'=>'点击预览数据'))} </a> {:btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','round'=>1,'url'=>url('website/datamod','tables='.$dshow['table'].'&martables='.$dshow['martable'].'&id='.$obj['Id'])))} </td>
              </tr>
            </volist>
            <tr>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-danger btn-sm" data-dismiss="modal">{:icon('off')} 关闭</button>
        </div>
      </div>
    </div>
  </div>
