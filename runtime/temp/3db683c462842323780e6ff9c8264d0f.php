    ...
    <div class="panel-body">
      <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="<?php echo tabstyle(); ?>">
        ...
        <?php if(is_array($data) || $data instanceof \think\Collection || $data instanceof \think\Paginator): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$obj): $mod = ($i % 2 );++$i;?>
          <tr class="maintr">
            <td align="left" valign="middle" height="35"><?php echo faicon('database'); ?> <?php echo $obj['path']; ?></td>
            <td align="left" valign="middle"><?php echo $obj['size']; ?></td>
            <td align="left" valign="middle"><?php echo $obj['time']; ?></td>
            <td align="left" valign="middle"> <?php echo btn(array('vals'=>'下载','icon'=>'download','tips'=>'点击下载此备份文件','round'=>1,'url'=>url('system/downdataup','downpath='.$obj['path'],''))); if($datasys['isdel'] == 1): ?> <a href="javascript:void(0)" data-path="<?php echo $obj['path']; ?>" class="deldataup"><?php echo btn(array('vals'=>'删除','icon'=>'trash','round'=>1,'tips'=>'点击删除该备份文件','scene'=>'danger')); ?></a> <?php endif; if($datasys['isre'] == 1): ?> <a href="javascript:void(0)" data-path="<?php echo $obj['path']; ?>" class="redataup"><?php echo btn(array('vals'=>'还原','icon'=>'refresh','round'=>1,'tips'=>'点击还原该份备份数据','scene'=>'success')); ?></a> <?php endif; ?></td>
          </tr>
        <?php endforeach; endif; else: echo "" ;endif; ?>
      </table>
      <div class="alert alert-danger" style="margin:10px auto;"><?php echo icon('warning-sign'); ?> 提示，任何形式删除的数据都无法找回，请谨慎操作！</div>
    </div>
  </div>
