(function(){
	...
	b.prototype = {
		init:function(){
			...
			t.w_width = $(".order_tpl").width();
			t.w_height = $(".order_tpl").height();
			...
			t.t.css({'max-height':t.w_height-t.c_height,'max-width':t.w_width-t.c_width,'position':'absolute'});
		...
		move:function(){
			...
			t.c.on('mousedown',function(e){
				...
				$(document).on('mousemove',function(e){
					t.left2 = t.left+e.pageX-t.x-$('.order_tpl').offset().left;
					t.top2 = t.top+e.pageY-t.y-$('.order_tpl').offset().top;
					if(t.left2 <=0){
						t.left2 = 0;
		...
		size:function(){
			var t= this;
			var warpbox = $('.order_tpl');
			var to_Y = warpbox.offset().top;
			var to_X = warpbox.offset().left;
			...
			t.sz.on('mousedown',function(e){
				...
				$(document).on('mousemove',function(e){
					...
					if(t.t.outerHeight() + t.top >= t.w_height+to_Y){
						t.t.height(t.w_height - t.top + to_Y);
					}
					if(t.t.outerWidth() + t.left >= t.w_width+to_X){
						t.t.width(t.w_width - t.left - t.c_width+to_X);
					}
					return false;
				});
